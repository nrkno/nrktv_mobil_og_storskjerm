...
#+begin_src emacs-lisp :exports results :results none
  ...
  (defun tusenskille-og-komma (s backend info)
    (let ((num (number-to-string 1234567.891011)))
      (string-match "\\([0-9]+\\)\\.\\([0-9]+\\)" num)
      (let ((hel (match-string 1 num))
  	  (dec (match-string 2 num)))
...
Er NRK TV på mobilen bare et verktøy for å spille av innhold på storskjerm? Fungerer egentlig mobilen bare som en fjernkontroll for NRK TV? La meg finne det ut.
